...
(defn login [cs username password captcha-apikey]
  "Login to mobage"
  (println username)
  (let [params (-> (http-get "http://sp.mbga.jp/_lg" cs)
                   :body
                   (e/html-snippet)
                   (as-> it
    ...
    (let [captcha (solve-captcha captcha-apikey (get-remote-image-bytes cs (params "captcha_url")))]
      ...
      (-> (http-post "https://connect.mobage.jp/login" cs {:form-params (into params {"captcha" captcha})})
          (:body)
          (println))))
  (-> (http-get "http://sp.pf.mbga.jp/12008305/?guid=ON&url=http%3A%2F%2F125.6.169.35%2Fidolmaster%2Fresults%2Findex%3Fl_frm%3DMypage_1%26rnd%3D956286586" cs)
      (:body)
      (e/html-snippet)
...
(defn logout [cs username password]
  "Logout from mobage"
  (-> (http-get "http://sp.mbga.jp/_logout_confirm" cs)))
